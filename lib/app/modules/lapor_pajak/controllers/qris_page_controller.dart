...
class QrisPageController extends GetxController {
  Api api;
  QrisPageController(this.api);
  late AuthModel authModel;
  late ModelGetpelaporanUser dataArgument;
  int? totalpajak;
  String $id_institution_qris =
      "230714011"; //9 digit id_institution khusus QRIS dari BPD
  ...
  Future CheckQRIS() async {
    ...
    var url_update = Uri.parse("${URL_APP}/vaqris/check_qris.php");
    var response = await http.post(url_update, body: {
      "nomor_kohir": "${dataArgument.nomorKohir}",
    });
    List data = json.decode(response.body);
    if (data[0]["message"] == "null") {
      //jika QRIS belum ada/belum pernah di generate untuk nomor_kohir ini
  ...
  Future AuthApiQRIS() async {
    isLoading = true;
    var url_auth = Uri.parse("${URL_APP}/vaqris/check_auth.php?kategori=qris");
    ...
    if (selisih > 3600000) {
      ...
      final body = {
        "username": "PDLBONTANG",
        "password": "PDLBONTANG"
      }; //{"username": "qrisdev", "password": "PB@|1Kp@paN19112021"}
      var url = Uri.parse(
      ...
      if (data['message'] == "success") {
        ...
        var url_update =
            Uri.parse("${URL_APP}/vaqris/update.php?kategori=qris");
        var response = await http.post(url_update, body: {
          "username": "PDLBONTANG",
          "password": "PDLBONTANG",
          "token": "${data['token']}",
        ...
        if (data_update == "Success") {
          ...
          var url = Uri.parse(
              "https://qris-pemda.bankaltimtara.co.id/api/qrismpm/generate");
          var response =
              await http.post(url, headers: headers, body: jsonEncode(body));
          ...
          if (data_QRIS['message'] == "success") {
            ...
            var url_update = Uri.parse("${URL_APP}/vaqris/create_qris.php");
            var response = await http.post(url_update, body: {
              "nomor_kohir": "${dataArgument.nomorKohir}",
              "amount": "${totalpajak}",
              "method": "12",
